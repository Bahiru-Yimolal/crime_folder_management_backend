...
const registerUserService = async (
  ...
  try {
    ...
    const existingSector = await Sector.findOne({ where: { sector_name: sector } });

        if (!existingSector) {
      // Throw a custom error if the user already exists
      throw new AppError("Sector with this name does not exists", 404);
    }
    ...
    // Re-throw the error as an operational error if it's a known error
...
const registerHeadService = async (
  first_name,
  last_name,
  email,
  phone_number,
...
const registerWeService = async (
  ...
  try {
    ...
    const roless = await Role.create({
      categories:"Admin",
      user_id:1,
...
const registerSuperService = async (
  first_name,
  last_name,
  email,
  phone_number,
...
const updateUserService = async (userId, firstName, lastName,email, phoneNumber) => {
  ...
  if (email) {
    const existingUser = await User.findOne({
      where: { email: email, user_id: { [Op.ne]: userId } },
    });
    if (existingUser) {
...
const loginService = async (phone_number, password) => {
  ...
  if (user.role === "user") {
    ...
    // const userCheck = await Profile.findOne({ where: { phoneNumber:phone_number } });
    // if (!userCheck){
    //   throw new AppError("User have no profile to see", 404);
    // }
    ...
    // For supAdmin or other roles
    if (user.status === "pending") {
      throw new AppError(
        "User must get Admin Permission before logging in",
        403
      );
    }
    ...
    const token = generateToken({
      userId: user.user_id,
      role: role ? role.categories : null,
      categoryId: role ? role.category_id : null,
    });
